...
def _setting(name, default=None):
    """Env var first, then Streamlit secrets (Streamlit Cloud can't set env vars)."""
...
API_URL = (_setting("API_URL") or "http://127.0.0.1:8000").rstrip("/")
...
def get_risk_score(result):
    return max(0, min(100, float(result.get("fraud_probability", 0))))
...
def render_risk_panel(result):
    ...
    if status == "PENDING":
        ...
        badge = "FRAUDULENT" if status == "FRAUDULENT" else "CLEARED"
    ...
    # Blank until we have a real answer, rather than showing a made up default.
    threshold_text = (
        f"{result['threshold']:.2f}%" if result.get("threshold") is not None else "--"
    )
    ...
    st.markdown(
        ...
        <div class="panel {border_class}">
            ...
            <div class="summary-row"><span>Fraud Probability</span><span class="summary-value">{confidence:.2f}%</span></div>
            <div class="summary-row"><span>Alert Threshold</span><span class="summary-value">{threshold_text}</span></div>
            <div class="summary-row"><span>Prediction Latency</span><span class="summary-value">{result.get("latency_ms", 0)} ms</span></div>
...
def render_attribution(result):
    ...
    if not factors:
        # Nothing to show yet, or SHAP could not run. Either way say so rather
        # than drawing bars from made up numbers.
        if result.get("decision"):
            reason = explanation.get("reason") or "no attribution returned"
            subtitle = f"The score is exact, but no breakdown was available: {reason}."
        else:
            subtitle = "Run a prediction to see which features drove the score."
        st.markdown(
            f"""
            <div class="panel">
...
zero_result = {
    ...
    "model": "XGBoost",
    "latency_ms": 0,
    "explanation": {"available": False, "factors": []},
}
...
if submitted:
    try:
        ...
        st.caption(f"Checked endpoint: {PREDICT_URL}")
        st.exception(exc)
